...
- 白ベースのモノクロ UI。カードはノートのフィールドから Kioku が組み立てる(単語・例文・意味・音声を 1 画面に。設定でデッキのテンプレート表示にも戻せる)
...
| 操作 | 意味 |
|---|---|
| 上に引く / タップ | 答えのパネルが指に付いて下から上がってくる |
| 答えが開いた状態でさらに上にスクロール | 普通(Good)で確定して次のカードへ。次のカードは下に先読みされています |
...
### ペースを決める(1章/週、30語/日、または期限)

デッキ画面 →「ペースを決める」。決め方は 2 つ。**ペース**(1章/週 など)と**期限**(節 X〜Y をこの日までに)。期限型は毎日「残り枚数 ÷ 残り日数」で新規を出し、遅れた分は翌日以降に上乗せされます。
単位(章 / 単語数)、量、期間(日 / 週 / 任意日数)、開始日、開始する章を指定します。
...
章モードにはサブデッキが必要です。無ければ先に「章に分ける」で分けてください。章の下にさらに節(サブデッキ)があるデッキでは「数える階層」で章/節を選べます(1節/日 など)。「各章を節に分ける」で章の中を N 枚ずつの節に切れます。
...
スケジューリングは本家の Rust コアが行います。新しいコレクションは Anki と同じく従来方式(SM-2)で始まるので、設定 → スケジューラ → **FSRS を使う** をオンにすると FSRS-6 に切り替わります(本家の「デッキオプション → FSRS」と同じ処理を呼びます)。
